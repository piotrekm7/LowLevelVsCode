 ...
 */
function generateWebviewPanel(viewType: string, title: string, context: vscode.ExtensionContext): vscode.WebviewPanel {
	function getPathTo(filename: string): string {
		const fileUri = vscode.Uri.file(path.join(context.extensionPath, 'src', 'frontend', viewType, filename))
	...
	fs.readFile(path.join(context.extensionPath,'src', 'frontend', viewType, 'index.html'),(err,data) => {
		if (err) {
			console.error(err)
			return;
		}
...
export function projectSettings(context: vscode.ExtensionContext): () => void {
	return () => {
		const panel = generateWebviewPanel('projectSettings', 'Project Settings', context);
